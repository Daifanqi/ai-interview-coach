...
**追加：英文WPM区间校准 + 脚本自身一个bug的修复。** 决策#46合并后，
用户又补录了3段英文脚本朗读（同一段约85词的文本，同样"放慢/正常/
说快"三种意图），加上之前那段自由发挥的英文录音，`wpm`维度凑够了
4个数据点。重新跑`scripts/calibrate_speech_features.py`时发现两件事：

1. **脚本本身的bug：** 输出里"Current bands"那一行打印的是我写死在
   代码里的字符串`cpm=(180, 260)`，没有跟着决策#46的调整同步更新——
   显示的是过时数字，但实际参与分类判断的`classify_speech_rate()`
   用的确实是`features.py`里真实的新常量（表格里对应几行显示的
   "normal"就是证据，用旧区间算不出这个结果）。也就是说这个bug只
   影响这一行提示文字，不影响真实的校准判断逻辑。修复：改成从
   `_RATE_BANDS`实时读取，不再硬编码。
2. **WPM区间调整：** 3段英文脚本朗读实测184/181/187，全部超过旧区间
   `fast_min=150`，跟中文那次的模式一样。但和中文不同的是，这三段
   按"慢/正常/快"意图读出来的实际差距很小（只差6，中文那次三段差了
   近70），说明这次数据里没有真正"明显更快"的例子可以用来锚定
   fast_min。调整为`(140.0, 200.0)`：`slow_max=140`是有数据支撑
   的——那段自由发挥的录音（109 wpm）明显比脚本朗读那组慢，调整后
   依然会被判"slow"；`fast_min=200`目前没有真实数据支撑，只是放在
   已观测到的最快值（187）之上留出余量的**占位猜测**，代码注释里
   明确写了这一点，等以后真的录到一段"确实说得快"（而不只是"想说快"）
   的英文，再回头看这个上限站不站得住。

**验证：** 校准脚本本身的转写+特征提取数据是用户本地真实跑出来的。
区间常量改动、脚本bug修复、`py_compile`语法检查是我这边做的，需要
用户本地重跑`pytest tests/`确认（`test_classify_speech_rate_buckets_
slow_normal_fast()`因为是动态从`_RATE_BANDS`读边界值断言的，理论上
不需要再改就能适配新区间，但仍需要真的跑一次确认）。

**状态：** 代码已完成，等待用户本地`pytest tests/`确认后提交合并
推送——同样是小追加改动，不单独算一"周"。
